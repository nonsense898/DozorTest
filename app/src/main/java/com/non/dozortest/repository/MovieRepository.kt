...
class MovieRepository @Inject constructor(
    ...
    fun isMovieSaved(movieId: Int): Flow<Boolean> = movieDao.isMovieSaved(movieId).map { it }
    fun getMovies(query: String = "", genre: Int? = 0): Flow<PagingData<Movie>> = Pager(
        config = PagingConfig(
            pageSize = 6,
    ...
    suspend fun fetchMovieVideos(movieId: Int): Response<VideoResponse> =
